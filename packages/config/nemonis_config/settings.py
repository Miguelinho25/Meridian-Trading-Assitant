...
from nemonis_config import limits
from nemonis_config.product import ENV_PREFIX
...
class Settings(BaseSettings):
    ...
    database_url: str = "sqlite+aiosqlite:///./var/nemonis.db"
    ...
    def _clamp_risk_ceiling(cls, v: Decimal) -> Decimal:
        ...
        if v > limits.MAX_RISK_PER_TRADE_PCT:
            raise ValueError(
                ...
                f"code change to nemonis_config.limits, not an environment variable."
            )
    ...
    def _validate_execution_safety(self) -> Settings:
        ...
        if self.mode is Mode.BROKER:
            raise ConfigurationError(
                "NEMONIS_MODE=broker is not supported. Live broker execution is not "
                "implemented in this build — there is no broker adapter to connect to. "
                "See docs/architecture.md §9."
            )
        if self.broker_execution_enabled:
            raise ConfigurationError(
                "NEMONIS_BROKER_EXECUTION_ENABLED=true, but no broker adapter exists. "
