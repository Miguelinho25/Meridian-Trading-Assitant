...
from meridian_vault.writer import slugify
...
def parse_note(text: str) -> ParsedNote:
    ...
    return ParsedNote(
        ...
        body=parts[2].lstrip("\n"),
        content_hash=frontmatter.get("meridian_content_hash", ""),
    )
...
def render_trade_note(
    ...
    *,
    ...
    frontmatter = render_frontmatter(
        {
            "meridian_id": trade.trade_id,
            "meridian_type": "trade",
            "meridian_generated_at": generated_at,
            "meridian_content_hash": content_hash,
            "meridian_schema": SCHEMA_VERSION,
            "meridian_editable": list(TRADE_EDITABLE_FIELDS),
...
def detect_conflicts(note: str, expected_hash: str) -> list[Conflict]:
    ...
    actual = parsed.frontmatter.get("meridian_content_hash", "")
