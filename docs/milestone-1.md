...
**Outcome:** eight steps complete, four partial (3, 4, 10, 11). Verified by hand
and by driving the running application rather than by an automated browser suite,
which is itself one of the gaps. See [handoff.md](handoff.md) for the step-by-step
position.
...
Milestone 1 delivered the apparatus. It did not deliver an edge, and the record
says so: 9 backtest runs, 0 qualifying as evidence; a 2010–2026 replay ending at
92,662 from 100,000. Both baselines remain CANDIDATE, and nothing is ACTIVE.

That is the intended result of a milestone whose explicit non-goal is below.
